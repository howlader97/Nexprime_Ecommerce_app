...
import 'package:nexprime/widgets/buttons/app_dropdown_field.dart';
...
class VendorHomeScreen extends ConsumerStatefulWidget {
  ...
  static const List<String> title = ["Total Revenue", "Pending Orders", "Total Products", "Total followers"];
...
class _VendorHomeScreenState extends ConsumerState<VendorHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final storeName = dashBoard.value is VendorTodayDashboardModel
        ...
        : (dashBoard.value is VendorSevenDaysDashboardModel ? (dashBoard.value as VendorSevenDaysDashboardModel).storeName : 'N/A');

    // Get raw earnings list based on model type
    final List<dynamic> rawEarnings;
    if (dashBoard.value is VendorTodayDashboardModel) {
      rawEarnings = (dashBoard.value as VendorTodayDashboardModel).earningsOverTime;
    } else if (dashBoard.value is VendorSevenDaysDashboardModel) {
      rawEarnings = (dashBoard.value as VendorSevenDaysDashboardModel).earningsOverTime;
    } else {
      rawEarnings = [];
    }
    ...
    if (selectedRange == 'today' || selectedRange == 'yesterday') {
      final labels = ["12-3 AM", "4-7 AM", "8-11 AM", "12-3 PM", "4-7 PM", "8-11 PM"];
      for (int i = 0; i < 6; i++) {
        double sum = 0;
        for (int j = 0; j < 4; j++) {
          int index = i * 4 + j;
      ...
      for (int i = 0; i < 6; i++) {
        ...
        if (startIdx < N) {
          ...
          final String label = selectedRange == 'last_1_year' ? _getYearRangeLabel(startLabel, endLabel) : _getRangeLabel(startLabel, endLabel);
    ...
    final double maxAmount = chartData.isNotEmpty ? chartData.map((e) => e.earnings).reduce((a, b) => a > b ? a : b) : 0;

    return SafeArea(
      child: RefreshIndicator(
        onRefresh: () async {
          await ref.read(vendorDashboard.notifier).getDashboardData();
        },
        child: CustomScrollView(
          ...
          slivers: [
            SliverToBoxAdapter(
              child: Padding(
                padding: EdgeInsets.symmetric(vertical: AppSize.size.height * 0.01, horizontal: AppSize.size.width * 0.03),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    AppText(text: "Hello, $storeName", fontSize: 24, fontWeight: FontWeight.bold, height: 1.5),
                    RichText(
                      text: TextSpan(
                        children: [
                          TextSpan(
                            text: "Your store ",
                            style: TextStyle(fontSize: 16, fontWeight: FontWeight.w400, height: 1.5, color: AppColors.instance.black900),
                          ),
                          TextSpan(
                            text: "$storeName ",
                            style: TextStyle(color: AppColors.instance.green, fontSize: 16, fontWeight: FontWeight.w400, height: 1.5),
                          ),
                          TextSpan(
                    ...
                    GridView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        ...
                        final double earning = val is VendorTodayDashboardModel
                            ? val.totalEarnings
                            : (val is VendorSevenDaysDashboardModel ? val.totalEarnings : 0.0);
                        final int pendingOrders = val is VendorTodayDashboardModel
                            ? val.totalPendingOrders
                            : (val is VendorSevenDaysDashboardModel ? val.totalPendingOrders : 0);
                        final int catalogSize = val is VendorTodayDashboardModel
                            ? val.totalProducts
                            : (val is VendorSevenDaysDashboardModel ? val.totalProducts : 0);
                        final int storeRating = val is VendorTodayDashboardModel
                            ? val.totalFollowers
                            : (val is VendorSevenDaysDashboardModel ? val.totalFollowers : 0);
                        final List<num> value = [earning, pendingOrders, catalogSize, storeRating];
                    ...
                    Row(
                      ...
                      children: [
                        const AppText(text: "Last sales trend", fontSize: 22, fontWeight: FontWeight.w600, height: 1.5),
                        Gap(width: 4),
                        Expanded(
                          child: AppDropdownField(
                            provider: vendorHomeDropdownProvider,
                            options: const ['today', 'yesterday', 'last_7_days', 'last_30_days', 'this_month', 'last_1_year', 'custom'],
                              ...
                              child: AppInputWidget(
                                ...
                                onTap: () {
                                  ...
                                  if (_startDateController.text.isNotEmpty) {
                                    try {
                                      initial = DateTime.parse(_startDateController.text);
                                    } catch (_) {}
                                  }
                                  showCustomCalendarView(
                                    ...
                                    onDateSelected: (date) {
                                      ...
                                      ref.read(vendorHomeStartDateProvider.notifier).state = formatted;
                                    },
                                  );
                                },
                              ),
                              ...
                              child: AppInputWidget(
                                ...
                                onTap: () {
                                  ...
                                  if (_endDateController.text.isNotEmpty) {
                                    try {
                                      initial = DateTime.parse(_endDateController.text);
                                    } catch (_) {}
                                  }
                                  showCustomCalendarView(
                                    initialDate: initial,
                                    ...
                                    onDateSelected: (date) {
                                      ...
                                      ref.read(vendorHomeEndDateProvider.notifier).state = formatted;
                                    },
                                  );
                                },
                              ),
                    ...
                    Container(
                      ...
                      height: 220,
                      decoration: BoxDecoration(color: AppColors.instance.dark50, borderRadius: BorderRadius.circular(10)),
                      child: Padding(
                        padding: EdgeInsets.symmetric(vertical: AppSize.size.height * 0.03, horizontal: AppSize.size.width * 0.03),
                        child: Row(
                          crossAxisAlignment: CrossAxisAlignment.end,
                          children: chartData.map((item) {
                            double heightFactor = maxAmount == 0 ? 0 : (item.earnings / maxAmount).clamp(0.0, 1.0);
                                        ...
                                        child: Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            gradient: const LinearGradient(
                                              ...
                                              colors: [Color(0xFFB9DFBE), Color(0xFF2E7D32)],
                                            ),
                                          ),
                                        ),
                                      ),
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            AppText(text: title, fontSize: 16, fontWeight: FontWeight.w600, height: 1.5),
            const Gap(height: 12),
            SizedBox(
...
String _getYearRangeLabel(String start, String end) {
  ...
  if (startParts.length == 2 && endParts.length == 2) {
    ...
    final shortStartYear = startYear.length >= 4 ? startYear.substring(2) : startYear;
