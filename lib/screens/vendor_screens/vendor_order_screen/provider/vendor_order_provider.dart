...
class VendorOrderNotifier extends AsyncNotifier<List<VendorOrderModel>> {
  ...
  Future<void> archiveSubOrder(int id, bool status) async {
    final success = await VendorOrderRepository.instance.archiveSubOrder(
      id,
      status,
    );
    if (success) {
  ...
  Future<void> completeSubOrder(int id, bool status) async {
    final success = await VendorOrderRepository.instance.completeSubOrder(
      id,
      status,
    );
    if (success) {
    ...
    data: (orders) {
      ...
      if (query.isNotEmpty) {
        filtered = filtered.where((o) {
          ...
          final matchesProduct = o.orderItems.any((item) =>
