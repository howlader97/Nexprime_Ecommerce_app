...
import '../../../widgets/dialogs/login_pop_up.dart';
...
class CustomerFoodDetailsScreen extends ConsumerWidget {
  ...
  List<String> get imageUrls => product.images.isNotEmpty ? product.images : ["https://ui-avatars.com/api/?name=No+Image&background=random"];
              ...
              flexibleSpace: Stack(
                children: [
                  ...
                  Positioned(
                    ...
                    child: IconButtonWidget(onTap: () => previousPage(currentPage), icon: Icons.chevron_left),
                  ),
                  Positioned(
                    right: 16,
                    top: AppSize.size.height * 0.2,
                    child: IconButtonWidget(onTap: () => _nextPage(currentPage), icon: Icons.chevron_right),
                  ),
                  Positioned(
                    bottom: 30,
                    left: 0,
                    ...
                    child: Row(
                      ...
                      children: List.generate(imageUrls.length, (index) {
                        ...
                        return AnimatedContainer(
                          ...
                          width: isActive ? 12 : 8,
                          height: 8,
                          decoration: BoxDecoration(shape: BoxShape.circle, color: isActive ? AppColors.instance.green : Colors.white.withAlpha(200)),
                        );
                      }),
                    ),
                  ),
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Expanded(
                              child: AppText(
                                text: product.categories.isNotEmpty ? product.categories.first.name : "Category",
                                fontSize: 18,
                                color: Colors.black,
                            ...
                            CustomDecoratedBox(
                              ...
                              child: Center(
                                child: AppText(text: '¥${product.salePrice.toStringAsFixed(2)}', color: AppColors.instance.white50),
                              ),
                            ),
                          ],
                        ),
                        Row(
                          children: [
                            AppText(
                              text: "Shipping: ${product.shippingResponsibility}",
                              fontSize: 18,
                              color: Colors.black,
                              overflow: TextOverflow.ellipsis,
                              maxLines: 1,
                           ...
                           Gap(width: 20,),
                           // if(product.shippingResponsibility != "VENDOR")
                              AppText(
                                text: "Charge: ${product.shippingCharge.toString()}",
                                fontSize: 18,
                                color: Colors.black,
                                overflow: TextOverflow.ellipsis,
                                maxLines: 1,
                          ...
                          children: [
                            reviewDetails.when(
                              data: (reviews) {
                                ...
                                return Row(
                                  children: [
                                    Icon(Icons.star, color: Colors.amber, size: AppSize.size.width * 0.055),
                                    Gap(width: AppSize.size.width * 0.012),
                                    AppText(text: avgRating.toStringAsFixed(1), fontWeight: FontWeight.bold, fontSize: AppSize.size.width * 0.046),
                                    Gap(width: AppSize.size.width * 0.012),
                                    GestureDetector(
                                      onTap: () {
                                        AppRoutes.instance.pushNamed(AppRoutesKey.instance.customerReviewListScreen, extra: product.id);
                                      },
                                      child: AppText(
                              ...
                              loading: () => SizedBox(height: 10, width: 10, child: Center(child: CircularProgressIndicator())),
                            ),

                            Spacer(),
                            Icon(Icons.circle, color: Colors.green, size: AppSize.size.width * 0.03),
                            Gap(width: AppSize.size.width * 0.02),
                            AppText(text: "${product.stockUnits} in stock", color: Colors.grey.shade600, fontSize: AppSize.size.width * 0.04),

                            Spacer(),
                            AppText(text: "Tax:${product.taxFee}%"),
                            // AppText(
                            //   text: "Copy link",
                            //   color: Colors.grey.shade600,
                            //   fontSize: AppSize.size.width * 0.04,
                        ...
                        Gap(height: AppSize.size.width * 0.02),
                        AppText(text: "Description", fontSize: AppSize.size.width * 0.048, fontWeight: FontWeight.bold),
                          ...
                          onTap: cartState.isLoading
                              ...
                              : () async {
                                  try {
                                    ...
                                    if (context.mounted) {
                                      AppSnackBar.instance.success("Data successfully added to cart");
                                    }
                        ...
                        ViewShopWidget(
                          onTap: () {
                            AppRoutes.instance.pushNamed(AppRoutesKey.instance.customerShopScreen, extra: product.storeId.toString());
                          },
                          title: product.store.name.isNotEmpty ? product.store.name : 'Store',
                          image: product.store.photo.isNotEmpty ? product.store.photo : AppAssertsIconsPath.instance.shopIcon,
                        ),
                      ],
                    ),
                  ),
