...
class CustomerNewProductList extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CustomerNewProductList> createState() =>
      _CustomerNewProductListState();
}

class _CustomerNewProductListState
    extends ConsumerState<CustomerNewProductList> {
  String? selectedType;
  ...
  void onAppInitial() {
    try {
      ...
      shippingCharge =
          TextEditingController(text: widget.product?.shippingCharge?.toString());

      if (widget.product != null) {
        selectedType = widget.product!.goodsType;
        shippingResponsibility =
            widget.product!.shippingResponsibility ?? "CUSTOMER";
      }
    } catch (e) {
      errorLog("Error is", e);
    }
  ...
  void onPublished() async {
    ...
    bool success = false;
    if (widget.product != null) {
      success = await MarketingProductRepository.instance
          .updateMarketingProduct(widget.product!.id!, data, selectedImages);
    } else {
      success = await MarketingProductRepository.instance
          .publishMarketingProduct(data, selectedImages);
    }
    ...
    //  await ref.refresh(myMarketingProductProvider.future);
     // await ref.refresh(marketingProductProvider.future);
  ...
  Widget build(BuildContext context) {
    ...
    final safeValue= typesOfGoods.contains(selectedType)
    ? selectedType: null;
    return Scaffold(
      body: SafeArea(
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: Padding(
                padding: EdgeInsets.symmetric(
                  horizontal: AppSize.size.width * 0.046,
                  vertical: AppSize.size.width * 0.046,
                ),
                child: isLoading
                    ...
                    : AppButton(
                        ...
                        title: widget.product != null ? "Update" : "Published",
                        onTap: onPublished,
                      ),
              ),
            ),
